#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run one-day historical daily factor calculation and compare with live output."""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="/tmp/hist_daily_compare.parquet")
    ap.add_argument("--securities-from-live", action="store_true")
    ap.add_argument("--product-precision", action="store_true")
    ...
    def outfun(date: str, end_time: str, df: pd.DataFrame) -> None:
        print(f"historical_out date={date} end_time={end_time} shape={df.shape}")
    ...
    if args.product_precision:
        _fcols = hist.select_dtypes(include=["float64", "float32"]).columns
        if len(_fcols) > 0:
            hist[_fcols] = hist[_fcols].round(6).astype("float32")
    hist.to_parquet(args.out, index=False)
    print(f"historical_saved={args.out} shape={hist.shape}")
